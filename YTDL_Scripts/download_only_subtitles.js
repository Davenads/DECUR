...
async function downloadSubtitlesOnly(videoUrl) {
    try {
        ...
        const outputTemplate = path.join(subtitlesPath, '%(title)s.%(ext)s');
        execSync(`"${ytDlpPath}" --cookies "${cookiesPath}" --write-auto-sub --sub-lang en --sub-format vtt --skip-download --output "${outputTemplate}" --verbose "${videoUrl}"`, { stdio: 'inherit' });
